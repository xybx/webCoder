...
export function setupPermissions(router: Router) {
  ...
  router.beforeEach(async (to, from, next) => {
    ...
    const { token, setVirtualRoles, setUserinfo, setUsername, setAvatar, resetAll } = useUserStore()
      ...
      if (routes.length) {
        ...
        try {
          ...
          if (loginInterception) {
            let json = JSON.parse(sessionStorage.getItem('userinfo') as string)
            setUserinfo(json)
            setUsername(json.name)
            setAvatar('./static/svg/avatar.svg')
